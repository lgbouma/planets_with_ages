...
DESCRIPTION
----------
An assortment of code written for sanity checks on our TESS GI proposal about
difference imaging of clusters.
...
def get_MWSC_stellar_data(df, savstr, p_0=61, datadir='../data/'):
    '''
    ...
    stars with kinematic and photometric membership probabilities >61%".  (See
    Kharchenko+ 2012 for definitions of these probabilities). It writes them to
    harddrive.
    ...
    readme = datadir+'stellar_data_README'

    # loop over clusters
    ix = 0
    for mwsc_id, name, r2 in list(zip(mwsc_ids, names, r2s)):

        outdir='../data/MWSC_1sigma_members/'
        outname='{:s}_{:s}_{:s}.csv'.  format(mwsc_id, str(name), savstr)
        if os.path.exists(outdir+outname):
            print('skip {:s}, found it'.format(outdir+outname))
            continue
        ...
        middlestr = str(mwsc_id) + '_' + str(name)
        fpath = datadir+'2m_'+middlestr+'.dat'
        if name != 'Melotte_20':
        ...
        c = SkyCoord(ra=np.array(members['RAhour'])*u.hourangle,
                     dec=np.array(members['DEdeg'])*u.deg)
        members['RAdeg'] = c.ra.value

        diff = abs(N_catalogd - N_my_onesigma)
        if diff > 5:
            print('\nWARNING: my cuts different from Kharachenko+ 2013!!')

        ascii.write(members, output=outdir+outname, format='ecsv',
                    overwrite=True)

        print('SAVED OUTPUT TO {:s}{:s}'.format(outdir,outname))

        ix += 1



def get_stellar_data_too(df, savstr, p_0=61, datadir='../data/'):
    '''
    args:
      savstr (str): gets the string used to ID the output pickle
